  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: '600px' }}>
          ...
          {allBreakpoints.map(bp => (
            <div
              ...
              style={{
                ...
                borderLeft: is(bp) ? '3px solid #3b82f6' : '3px solid transparent',
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.875rem' }}>
          {allBreakpoints.map(bp => (
            ...
            >
              {isAtLeast(bp) ? '✓' : '✗'} isAtLeast('{bp}') = {String(isAtLeast(bp))}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.875rem' }}>
          {allBreakpoints.map(bp => (
            ...
            >
              {isAtMost(bp) ? '✓' : '✗'} isAtMost('{bp}') = {String(isAtMost(bp))}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>💡 Try it:</strong> Resize your browser window to see the breakpoint detection update in real-time!
      </div>
    </div>
  );
};
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'React hook for detecting the current responsive breakpoint. Provides helper methods to check if the viewport is at least, at most, or exactly at a specific breakpoint. Updates automatically when the window is resized.'
      }
    }
  },
  tags: ['autodocs'],

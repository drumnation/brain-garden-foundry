...
export async function deployDestroyCommand(options: any) {
  ...
  if (!options.force) {
    ...
    console.log(chalk.gray(`  • Database collections: ${config.appwrite.collections.length}`));
    console.log(chalk.gray(`  • Storage buckets: ${config.appwrite.buckets.length}`));
    ...
    if (config.environment === 'production') {
      const { confirmProduction } = await prompts({
        ...
        validate: (value) => value === config.project || 'Project name does not match',
      });
  ...
  const steps = [
    {
      ...
      fn: async () => {
        if (options.keepData) {
          const backupDir = path.join(process.cwd(), '.deploy', 'backups', new Date().toISOString());
